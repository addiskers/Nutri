...
export const authService = {
  ...
  /**
   * Initiate Azure AD login - Get authorization URL
   */
  async getAzureLoginUrl() {
    try {
      ...
      if (response.ok) {
        return { success: true, auth_url: result.auth_url, state: result.state }
      } else {
        return { 
          success: false, 
          error: result.detail || 'Failed to get Azure login URL' 
        }
      }
    } catch (error) {
      return { success: false, error: 'Network error. Please try again.' }
  ...
  /**
   * Handle Azure AD callback - Exchange code for tokens
   */
  async azureCallback(code, state) {
    try {
